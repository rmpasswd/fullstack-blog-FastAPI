...
def  home(request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    r = db.execute(select(models.Post))
    posts = r.scalars()
...
def post_page(post_id: int, request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    r = db.execute(select(models.Post).where(models.Post.id==post_id))
...
def user_posts_page(user_id: int, request: Request, db:  Annotated[Session, Depends(get_db)]):
    ...
    if user:
        r = db.execute( select(models.Post).where(models.Post.user_id == user_id) )
        return templates.TemplateResponse(
...
def posts_all(db: Annotated[Session, Depends(get_db)]):
    posts = db.execute(select(models.Post)).scalars().all()
    return posts

@app.get("/api/posts/{post_id}", response_model=PostResponse)
def post_get(post_id: int , db: Annotated[Session, Depends(get_db)]):
...
def post_create(post: PostCreate,  db: Annotated[Session, Depends(get_db)]):

    # From schema, we see that the request should have a title, content and user_id

    # First get the User ID
    r = db.execute(select(models.User.id).where(models.User.id== post.user_id))
    ...
    db.add(new_post_object)
    db.commit()
    db.refresh(new_post_object)
    return new_post_object

@app.put("/api/posts/{post_id}", response_model=PostResponse)
def post_update_full(post_id: int , post_data: PostCreate, db: Annotated[Session, Depends(get_db)]):

    existing_post  = db.execute(select(models.Post).where(models.Post.id==post_id)).scalars().first()
    ...
    if post_data.user_id != existing_post.user_id:
        userr = db.execute( select(models.User.id).where(models.User.id == post_data.user_id) ).scalars().all()
        if not userr:
            raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail="User not found")
        ...
        db.commit(); db.refresh(existing_post)
...
def create_user(userr: UserCreate, db: Annotated[Session, Depends(get_db)]):
    
    existing_user = db.execute(select(models.User).where(models.User.username == userr.username))
    if existing_user.scalars().first(): # is not None
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Username already exists")
    
    existing_email = db.execute(select(models.User).where(models.User.email == userr.email))
    ...
    db.refresh(new_user) # It's a good habit when you have server-side defaults, triggers, etc. refresh() Fetches the latest state from the database to ensure your object reflects what's actually stored.

    return new_user

@app.patch("/api/users/{user_id}", response_model=UserResponse, status_code=status.HTTP_201_CREATED)
def update_partial_user(user_id: int, user_update: UserUpdate, db: Annotated[Session, Depends(get_db)]):
    
    existing_user = db.execute(select(models.User).where(models.User.username == userr.username)).scalars().first()
    ...
    emaill = db.execute(select(models.User).where(models.User.email == user_update.email)).scalars().first()
...
def get_users_all(db: Annotated[Session, Depends(get_db)]):
    r = db.execute( select(models.User)).scalars().all()
    if r:
        return r
    else:
        return []
...
def get_user(userid: int, db: Annotated[Session, Depends(get_db)]):
    ...
    dbquery = db.execute( select(models.User).where(models.User.id == userid) )
...
def delete_user(user_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    userr = db.execute( select(models.User).where(models.User.id == user_id) ).scalars().first()
    if not userr:
...
def get_user_posts(userid: int, db: Annotated[Session, Depends(get_db)]):
    ...
    r = db.execute(select(models.User).where(models.User.id==userid))
    if  r.scalars().first(): # Does the user exist?

        # User-specific posts
        r= db.execute( select(models.Post).where(models.Post.user_id == userid))
